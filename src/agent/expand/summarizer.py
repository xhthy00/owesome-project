...
2. **关键指标**：Markdown 表格 `| 指标 | 数值 |`，优先收录输入中已出现的：
   ...
   输入里已有各科/对比/排名/分段等结果表时，**必须保留整表**（可加在关键指标之后），
   表头改成中文（学科、均分、全市排名、参考人数、及格率），禁止照抄 scope/avg_score，
   禁止把整表压成一句话。
...
先用 1~3 句直接回答用户问题，文风像答疑，不要像写报告。
- **禁止**套用「学情总判 / 关键指标 / 学情解读 / 教学建议 / 报告指引」等固定章节或同名小标题；
- 输入里已有 Markdown 结果表（各科排名、校际对比、分段、达线等）时，**必须保留整表**；
  表前用一两句点题即可，表头用中文（学科、均分、全市排名、参考人数、及格率），
  **禁止**照抄 scope/avg_score/ref_count；不要另造数字，不要压成一句话；
- **禁止**主动写「参考人数 / 共 N 人参考 / 班级人数 / 权威统计口径」——
  本题未问全班人数；查询返回行数、Top-N、LIMIT 行数都**不是**班级人数；
- 若问「最好/最高/谁」：写出 student_id 与分数（及并列情况）即可，可附满分作对照；
- 若问「均分/多少人/排名」：只回答所问指标；有结果表则附表，勿扩写学情解读与教学建议；
- 若问「优势/薄弱学科」：必须照抄输入中的权威判定；全市前25%才是优势、后50%才是薄弱；
  全市第7/37这类前列名次**禁止**写成薄弱，即使它是本校各科里相对靠后的；
...
- 不要提及 SQL、表名（tb_*）、FILTER、字段名、工具、Agent、子任务等实现细节；
  口径只写「在籍生」「排除未选考」等口语。
...
class SummarizerAgent(ConversableAgent):
    profile = ProfileConfig(
        ...
        constraints=[
            ...
            "输入中的对比/排名/各科结果表须保留为 Markdown 表格，表头用中文，禁止压成一句话",
            "优势/薄弱学科须按全市排名相对位置：前25%才是优势，后50%才是薄弱；全市前列不得写成薄弱",
            "避免空泛鸡汤与实现细节（表名/SQL/FILTER/Agent/工具）",
