...
export default function ChatContentContainer({ messages, steps = [], selectedStepId, onSelectStep }: Props) {
  ...
  const buildRunEntry = (
    ...
    const realPlanSteps = mergedSteps.filter((s) => s.section === "plan" && !s.id.startsWith("plan-bootstrap"));
    const hasRealProgress = mergedSteps.some((s) => s.section !== "plan" || s.id.startsWith("tool-"));
    const planSteps = hasRealProgress ? realPlanSteps : mergedSteps.filter((s) => s.section === "plan");
    const toolSteps = mergedSteps.filter((s) => s.id.startsWith("tool-"));
    const derivedPlanSteps = (() => {
      ...
      const bySubTask = Array.from(subTaskSet)
        .sort((a, b) => a - b)
        .map((subTaskIdx) => ({
          id: `derived-plan-${msg.id}-${subTaskIdx}`,
      ...
      if (mergedSteps.length > 0) {
        return [
          {
            id: `derived-plan-fallback-${msg.id}`,
    ...
    const planProgress = derivedPlanSteps.length
      ? Math.round((doneCount(derivedPlanSteps) / derivedPlanSteps.length) * 100)
      : 0;
    return {
      message: msg,
      mergedSteps,
  ...
  const runEntriesFromUser = userMessages.map((msg, idx) => {
    const runSteps = steps.filter((s) => s.runId && s.runId === msg.runId);
    const mergedSteps = idx === userMessages.length - 1 ? [...runSteps, ...stepWithoutRunId] : runSteps;
    return buildRunEntry(msg, mergedSteps);
  });
  const runEntries = (() => {
    if (runEntriesFromUser.length > 0) return runEntriesFromUser;
    ...
    return [buildRunEntry(fallbackMessage, steps)];
  })();
                ...
                <div className="rounded-2xl border border-[#e7eaf0] bg-white p-4 shadow-[0_1px_3px_rgba(16,24,40,0.04)] dark:border-[#2f3441] dark:bg-[#111723]">
                  ...
                  <section className="rounded-xl border border-[#cbe2ff] bg-[#eaf4ff] dark:border-[#2f4b75] dark:bg-[#17263b]">
                    <button onClick={() => toggleGroup("plan")} className="flex w-full items-center gap-2 px-3 py-2 text-left">
                      ...
                      <span className="ml-auto text-xs font-semibold text-[#1677ff]">{entry.planProgress}%</span>
                      <span className="text-xs text-[#667085]">{entry.derivedPlanSteps.length} 步</span>
                    </button>
                    <div className="px-3 pb-2">
                      <div className="h-1.5 overflow-hidden rounded-full bg-[#dbe7ff] dark:bg-[#2a3c61]">
                        <div className="h-full rounded-full bg-[#3b82f6] transition-all" style={{ width: `${entry.planProgress}%` }} />
                      </div>
                    </div>
                    {expandedGroups.plan ? (
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <div className="flex shrink-0 items-center gap-2 text-[11px] text-[#667085] dark:text-[#94a3b8]">
                                ...
                                <span>{"rowCount" in step && typeof step.rowCount === "number" ? `共 ${step.rowCount} 行` : "--"}</span>
                              </div>
                            </div>
                            {step.detail ? (
                  ...
                  <section className="mt-3 rounded-xl border border-[#e7eaf0] bg-[#f8fafc] dark:border-[#2f3441] dark:bg-[#0f141f]">
                    <button onClick={() => toggleGroup("step")} className="flex w-full items-center gap-2 px-3 py-2 text-left">
                      ...
                      <span className="ml-auto text-xs text-[#667085]">{entry.toolCount} 次</span>
                    </button>
                    {expandedGroups.step ? (
                      <div className="space-y-2 px-3 pb-3">
                        {entry.toolGroups.map(([subTaskKey, items]) => (
                          <div key={`tool-${entry.message.id}-${String(subTaskKey)}`} className="space-y-1.5">
                            <div className="text-xs font-medium text-[#98a2b3] dark:text-[#64748b]">
                              {subTaskKey === "none" ? "全局" : `子任务 ${Number(subTaskKey) + 1}`}
                              {subTaskKey !== "none" && entry.derivedPlanSteps[Number(subTaskKey)]
                                ? `：${toText(entry.derivedPlanSteps[Number(subTaskKey)].title).replace(/^计划 \d+:\s*/, "")}`
                                : ""}
                            </div>
                            {items
                              .slice()
