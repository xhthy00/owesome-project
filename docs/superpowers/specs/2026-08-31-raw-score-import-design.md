...
- 宽表选科以 `xkkm` 为准：未选考即使科目列为 0 也不写该科 `tb_score`；语数英仍按列有值写入
- 小题分科目名与所选卷不一致 → **警告可继续**
...
| 6 | 数值列：科目分——语数英空单元格跳过该科；物化生史政地以 `xkkm` 判断是否参考（未选考列的 0 不写 `tb_score`、不校验超满分）；已选考非数值 / 为负 / 超该科满分（对照 `tb_exam.exam_score`）→ 阻断 | 错误行 |
