...
3. 有命中则只对最弱 1～2 科下钻小题（优先本班相对本校其他班失分的题），再给建议。
...
3. 纯函数算出位置表 + 薄弱列表。
4. 若薄弱非空：对最弱 1～2 科调用已有 `_fetch_subject_diagnosis_rows`（**不传 class_name**），再算班际小题差。
...
有命中：按 `(school_avg - class_avg)` 降序，取前 `DRILL_TOP_N` 科作为下钻对象；其余命中科仍出现在位置表，文案标「薄弱（本次未下钻题目）」。
