...
2. 导入成绩宽表（`.xlsx`）。写入 `tb_score_overview` / `tb_student` / `tb_score`。成功后扫描异常提醒，**不重算达线**。
3. 逐科导入小题分（`.xls` / `.xlsx`）。写入 `tb_score_detail`。学生必须已出现在本批次宽表中（按身份证号匹配），没有学校列兜底。小题分导入后不扫异常。
