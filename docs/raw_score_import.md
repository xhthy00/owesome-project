...
2. 导入成绩宽表（`.xlsx`）。界面填写届次（四位年份）：优先从宽表文件名中的「xxxx届」提取，也可手改；批次名仅作兜底。预览/导入后的分页列表在班级后展示届次。写入 `tb_score_overview`（含 `jc`）/ `tb_student` / `tb_score`。`tb_score` 中语数英有分即写；物化生史政地以 `xkkm` 判断是否参考（未选考列填 0 也不落该科）。成功后扫描异常提醒，**不重算达线**。重进页面时若该批次宽表已在库中，可跳过此步直接去导入小题分；重新上传仍会 UPSERT 覆盖（不会删除已有未选考残留行）。
